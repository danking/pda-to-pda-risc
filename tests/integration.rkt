...
(define-test-suite
  integration-tests
  (test-case
   "ab-matching grammar; e.g. (A A B B) => 2"
   (check-equal? (eval
                  `(,(compile-pdarisc
                      (compile-pda
                       (parse-pda ab-matching))
                      '(lambda (x) x)
                      'car
                      'cdr)
                    '(A A A A A B B B B B))
                 ...
                 '(10)
                 "(A A A A A B B B B B) => 10"))
  (test-case
   "infix calculator grammar; e.g. (2 + 3 * 3) => 11"
   (check-equal? (eval
                  `(,(compile-pdarisc
                      (compile-pda
                       (infer-pda-types
                        (parse-untyped-pda calculator-pda)))
                      '(lambda (t)
                         (if (number? t)
                             'NUM
                             (case t
