...
(check-syntax-equal?
 ...
 (make-pda (list #'A #'B #'$eos)
           ...
           (list (make-state #'s1 '(())
                 ...
                 (make-state #'s2 (list (list #'A #'s1)
                                        (list #'A #'s2))
                             (list (make-shift (list #'A) #'s2)
                                   (make-shift (list #'B) #'s3))
                             (list)
                             (list (make-goto 'start #'s4)))
                 (make-state #'s3 (list (list #'B #'s2 #'A #'s1)
                                        (list #'B #'s2 #'A #'s2))
                             (list (make-reduce '() #'r2))
                             (list)
                             (list))
                 (make-state #'s4 (list (list #'start #'s4 #'A #'s1)
                                        (list #'start #'s4 #'A #'s2))
                             (list (make-shift (list #'B) #'s5))
                             (list)
                             (list))
                 (make-state #'s5 (list (list #'B #'s5 #'start #'s4 #'A #'s1)
                                        (list #'B #'s5 #'start #'s4 #'A #'s2))
                             (list (make-reduce '() #'r1))
                             (list)
                             (list))
                 (make-state #'s6 (list (list #'start #'s1))
                             (list)
                             (list (make-accept (list #'$eos)))
                             (list)))
           (list (make-rule #'r1 (list (list #'B #'s5 #'start #'s4 #'A #'s2))
 ...
 (make-pda (list #'NUM
           ...
           (list (make-state
                 ...
                 (make-state
                  ...
                  (list (make-reduce (list #'*EOF*) #'r3))
                  (list)))
           (list (make-rule #'r4 #f 'program (list #'s-list #f)
                            #'(cons (if #f #f) s-list))
                 (make-rule #'r5 #f 's-list '() #''())
                 (make-rule #'r9 #f 'exp (list #'NUM) #'NUM)
                 (make-rule #'r13 #f 'exp (list #'expA #f #'exp)
                            #'(quotient expA exp))
                 (make-rule #'r14 #f 'exp (list #f #'exp #f) #'exp))))
