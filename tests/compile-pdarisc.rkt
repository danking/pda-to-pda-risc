...
;; Pure-RHS
(check-syntax-equal? (compile-pure-rhs (make-state #'s1) #'tr #'regs)
                     #'(make-state 's1))
(check-syntax-equal? (compile-pure-rhs (make-nterm #'foo) #'tr #'regs)
                     #'(make-nterm 'foo))
(check-syntax-equal? (compile-pure-rhs (make-curr-token #f) #'tr #'regs)
                     #'tr)
(check-syntax-equal? (compile-pure-rhs (make-named-reg #'some-var) #'tr #'regs)
                     #'(dict-ref regs 'some-var))
(check-syntax-equal? (compile-pure-rhs (make-nameless-reg) 'tr 'regs)
...
(check-syntax-equal? (compile-insn-seq*/default
                      (list (make-assign (make-named-reg #'v)
                                         (make-curr-token #f))
                            dummy-accept))
                     #`(lambda (in tr regs stack)
                         (#,dummy-accept/racket in
...
(check-syntax-equal? (compile-insn-seq*/default (list (make-block*
                                                       (list
                                                        (make-get-token)
                                                        (make-go
                                                         (make-label-name #'s0)
