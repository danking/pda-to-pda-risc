     ...
     (let ((rto-states (dict-ref rto-table nt)))
       (make-block*
        `(,@(compile-rule-args args)
          ,(make-sem-act (map make-reg-name (filter (lambda (x) x) args))
...
(define (compile-rule-args args)
  (if (empty? args)
      ...
      (foldl (lambda (x xs)
               (if x
                   (list* (make-assign (make-reg-name x) (make-pop))
                          (make-assign (make-reg-name '_) (make-pop))
                          xs)
                   (list* (make-assign (make-reg-name '_) (make-pop))
                          (make-assign (make-reg-name '_) (make-pop))
                          xs)))
             (list (make-assign (make-reg-name (if (first args)
                                                   (first args)
                                                   '_))
                                (make-pop))
                   (make-assign (make-reg-name 'target) (make-pop)))
